...
export class YandexDiskComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.oAuthService.setConfiguration(
      environment.oAuth.yandexDisk as OAuthConfiguration
    );
  }
  ...
  ngOnDestroy(): void {
    this.subscriptions.add(
      from(this.yandexDiskHubService.stopHubConnection()).subscribe({
        next: () =>
          this.toastService.information("Disconnected from SignalR hub."),
        error: () =>
          this.toastService.information(
            "An error has occurred while disconnecting to SignalR hub."
          ),
      })
    );
  ...
  startStopProcessing() {
    if (!this.isRunning) {
      ...
      const startProcessingSub = this.yandexDiskService
      ...
      const stopProcessingSub = this.yandexDiskService
  ...
  private getUserData(): void {
    const getUserSub = this.userService.getUser(this.userId).subscribe({
      next: (user) => {
        ...
        if (
          Date.now() < new Date(this.user.yandexDiskTokenExpiresOn).getTime()
        ) {
          this.needsAuthorization = false;
          this.tokenExpires = new Date(
            this.user.yandexDiskTokenExpiresOn
          ).toLocaleString();
        }
      },
      error: () =>
  ...
  private onRouteChanged(): void {
    const routeSub = this.activatedRoute.fragment
      .pipe(
        switchMap((fragment) => {
          if (fragment) {
      ...
      .subscribe({
        ...
        error: () =>
          this.toastService.information(
            "An error has occurred while parsing URL."
          ),
      });
  ...
  private startHub() {
    ...
    const startHubConnectionSub = from(
      this.yandexDiskHubService.startHubConnection()
    )
      .pipe(
        switchMap(() => {
  ...
  private subscribeToErrorEvent() {
    const errorSub = this.yandexDiskHubService
      .yandexDiskError()
      .pipe(
        switchMap((error) => {
  ...
  private subscribeToProgressEvent() {
    const progressSub = this.yandexDiskHubService
      .yandexDiskProgress()
      .subscribe({
        next: (progress) => {
